                ...
                <div className="min-w-max min-h-min invisible absolute bottom-0 top-[100%] flex flex-col rounded-md bg-richblack-5 p-2 text-richblack-900 opacity-0 transition-all duration-200 group-hover:visible group-hover:opacity-100">
                  {
                    subLinks?.map((subLink, index)=>{
                      return (
                        <li className="hover:bg-richblack-300 p-2 rounded-md"><Link to={subLink.name.toLowerCase().replace(" ", "-")}>{subLink.name}</Link></li>
                      )
                    })
                  }
                </div>
          ...
          <div className="flex gap-x-4 items-center">
            ...
            }
            {
                token === null && (
                    <Link to={"/login"}>
                        <button
                            className="bg-richblack-700 px-3 py-1 rounded-md active:shadow-white active:shadow-sm"
                        >Login</button>
                    </Link>
                )
            }
            ...
            }
            {
                token !== null && (
                    ...
                    <Link to={"/dashboard/my-profile"}>
                        <button
                            className="bg-richblack-600 px-3 py-1 rounded-md active:shadow-white active:shadow-sm"
                            >Dashboard</button>
                    </Link>
                    <Link to={"/"}>
                        <button
